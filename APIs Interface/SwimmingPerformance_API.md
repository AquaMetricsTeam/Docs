...
## 1. Create Swimming Performances

### Request

```http
POST /api/Swimming-Performance
```

### Description

Creates one or more swimming performance entries for an existing training record.

The request contains the `TrainingRecordId` and a list of swimming drills/performances.
  ...
  "swimmingPerformances": [
    {
      ...
      "rpe": 7,
      "status": 1,
      "coachComment": "Good pace and strong finish."
    }
  ]
}
```
...
|---|---|---:|---|
| `trainingRecordId` | `int` | Yes | Training record to which the swimming performances belong |
| `swimmingPerformances` | `array` | Yes | List of swimming drill/performance entries |

### Swimming Drill Fields

| Field | Type | Required | Description |
|---|---|---:|---|
| `stroke` | `StrokeType` | Yes | Swimming stroke or drill category |
| `distanceMeters` | `int` | Yes | Distance of the drill/set in meters |
| `repetitions` | `int` | Yes | Number of repetitions |
| `restIntervalSeconds` | `int` | Yes | Rest interval between repetitions |
| `bestRepTime` | `TimeSpan` | Yes | Best repetition/lap time |
| `averageRepTime` | `TimeSpan` | Yes | Average repetition/lap time |
| `worstRepTime` | `TimeSpan` | Yes | Worst repetition/lap time |
| `technique` | `PerformanceGrade` | Yes | Overall technique evaluation |
| `start` | `PerformanceGrade` | Yes | Start performance evaluation |
| `turns` | `PerformanceGrade` | Yes | Turn performance evaluation |
| `finish` | `PerformanceGrade` | Yes | Finish performance evaluation |
| `paceConsistency` | `PerformanceGrade` | Yes | Consistency of pace across repetitions |
| `rpe` | `int?` | No | Rate of Perceived Exertion |
| `status` | `PerformanceStatus` | Yes | Completion status |
| `coachComment` | `string?` | No | Coach comment |
